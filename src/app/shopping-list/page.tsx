...
export default function ShoppingListPage() {
  ...
  const [categorizedIngredients, setCategorizedIngredients] = useState<CategorizedIngredient[]>([]);
  ...
  const getGroupedIngredients = () => {
    return categorizedIngredients.reduce((acc, { ingredient, category }) => {
      if (!acc[category]) {
        acc[category] = [];
      }
          ...
          <div className="space-y-4">
            {recipes.map((recipe) => (
              <Card key={recipe.id} className="cursor-pointer hover:shadow-md transition-shadow">
                <CardHeader className="p-4" onClick={() => addRecipeToList(recipe)}>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                {selectedRecipes.map((recipe) => (
                  <div key={recipe.id} className="flex items-center justify-between bg-muted p-2 rounded">
